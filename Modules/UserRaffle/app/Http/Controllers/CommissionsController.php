...
class CommissionsController extends Controller
{
    ...
    public function index(): JsonResponse
    {
        //
        $user = auth()->user();
         $commisions = Commissions::join('raffles', 'raffles.id' ,'=' ,'commissions.raffles_id')
         ->join('users','users.taxid','=', 'raffles.user_taxid')
         ->select('commissions.*')
         ->with(['raffle','user'])
         ->where('users.taxid', $user->taxid)
         ->orderBy('commissions.created_at','ASC')
         ->paginate(10);
